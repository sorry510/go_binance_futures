...
### 发现的问题总结
1. 合约界面的价格有延迟: 原因是程序使用 websocket 订阅了币价变化的推流信息，网络不好的情况下(例如 本部 vpn 不稳定)会有延迟
2. 账户无法开仓: 币安 api 禁止香港和美国的 ip 方法，请换用其它地区 ip
3. 页面访问很慢: 请换用 mysql 数据库，sqlite 数据库性能不太行
4. api 报错: 币安的 api 方法有频率限制，需要降低访问 API 的频率
