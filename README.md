...
| 菜单 | 页面与用途 |
| --- | --- |
| 配置中心 | 管理合约交易、抢新、币种提醒、市场监听、资金费率监听、通知通道、调试推送和外部链接等非 AI 全局配置；基础 WebSocket 为系统强制启用，不提供关闭开关 |
...
1. 在 `配置中心 → 合约交易` 打开合约交易总开关。Futures/Spot 行情 WebSocket 与 Futures User Data WebSocket 属于系统基础设施，会自动启动，不提供关闭开关。合约交易总开关切换后直接保存，不再弹出二次确认框。
...
在 `配置中心 → 合约交易` 打开 `测试策略`。基础 WebSocket 会由系统自动启动。模拟交易遵循真实自动交易的策略和限制，但不会操作真实合约账户。结果可通过配置中心的 `查看测试结果` 按钮，或左侧 `合约交易 → 测试结果` 查看。
...
## 使用注意事项
- 网络必须能够正常访问 Binance；Binance HTTP 与 WebSocket 共用代理池配置。基础行情/User Data WebSocket 会自动启动并持续重连，网络或代理不稳定会触发相应 REST fallback/健康检查。
...
1. **登录后从哪里开启自动交易？** 进入 `配置中心 → 合约交易`，检查合约交易总开关和 `允许做多/允许做空`；再到 `合约交易 → 合约交易` 检查目标币种是否启用。基础 WebSocket 自动运行，无需人工开启。
...
4. **合约页面价格为什么有延迟？** 价格通过强制启用的 WebSocket 更新，网络或代理不稳定时会延迟。请检查系统看板中的 WebSocket freshness/错误状态和网络连接。
