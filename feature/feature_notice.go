...
func NoticeAndAutoOrder(systemConfig models.Config) {
	...
	for _, coin := range coins {
		...
		if coin.AutoOrder == 1 && canOrder {
			...
			if coin.Side == "buy" {
				_, err := submitNoticeAutoOpen(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, quantity, futures.SideTypeBuy, futures.PositionSideTypeLong)
				if err != nil {
					...
					if coin.ProfitPrice != "0" {
						...
						if managedQty, qtyErr := noticeManagedQuantity(coin.Symbol, futures.PositionSideTypeLong); qtyErr != nil {
							logs.Error("notice auto order load managed qty for take profit:", coin.Symbol, qtyErr)
						} else if _, protectErr := submitNoticeProtection(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, managedQty, profit_price_float64, futures.SideTypeSell, futures.PositionSideTypeLong, futuresownership.IntentTakeProfit); protectErr != nil {
							logs.Error("notice auto order take profit:", coin.Symbol, protectErr)
						}
					}
					if coin.LossPrice != "0" {
						// 挂一个止损单
						...
						if managedQty, qtyErr := noticeManagedQuantity(coin.Symbol, futures.PositionSideTypeLong); qtyErr != nil {
							logs.Error("notice auto order load managed qty for stop loss:", coin.Symbol, qtyErr)
						} else if _, protectErr := submitNoticeProtection(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, managedQty, loss_price_float64, futures.SideTypeSell, futures.PositionSideTypeLong, futuresownership.IntentStopLoss); protectErr != nil {
							logs.Error("notice auto order stop loss:", coin.Symbol, protectErr)
						}
					}
				}
			} else if coin.Side == "sell" {
				_, err := submitNoticeAutoOpen(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, quantity, futures.SideTypeSell, futures.PositionSideTypeShort)
				if err != nil {
					...
					if coin.ProfitPrice != "0" {
						...
						profit_price_float64, _ := strconv.ParseFloat(coin.ProfitPrice, 64)                 // 交易金额
						profit_price_float64 = utils.GetTradePrecision(profit_price_float64, coin.TickSize) // 合理精度价格
						if managedQty, qtyErr := noticeManagedQuantity(coin.Symbol, futures.PositionSideTypeShort); qtyErr != nil {
							logs.Error("notice auto order load managed qty for take profit:", coin.Symbol, qtyErr)
						} else if _, protectErr := submitNoticeProtection(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, managedQty, profit_price_float64, futures.SideTypeBuy, futures.PositionSideTypeShort, futuresownership.IntentTakeProfit); protectErr != nil {
							logs.Error("notice auto order take profit:", coin.Symbol, protectErr)
						}
					}
					if coin.LossPrice != "0" {
						...
						if managedQty, qtyErr := noticeManagedQuantity(coin.Symbol, futures.PositionSideTypeShort); qtyErr != nil {
							logs.Error("notice auto order load managed qty for stop loss:", coin.Symbol, qtyErr)
						} else if _, protectErr := submitNoticeProtection(fmt.Sprintf("notice_auto_order:%d", coin.ID), coin.Symbol, managedQty, loss_price_float64, futures.SideTypeBuy, futures.PositionSideTypeShort, futuresownership.IntentStopLoss); protectErr != nil {
							logs.Error("notice auto order stop loss:", coin.Symbol, protectErr)
						}
					}
				}
