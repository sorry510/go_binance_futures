...
func toModel(item *Task) models.AgentTask {
	row := models.AgentTask{
		...
		PromptHash: item.PromptHash, ModelConfigID: item.ModelConfigID, FinalModelConfigID: item.FinalModelConfigID,
		RouteCandidatesJSON: stringPointer(sanitizePayload(string(item.RouteCandidates))), RouteReason: stringPointer(sanitizeText(item.RouteReason)), RouteFallbackJSON: stringPointer(sanitizePayload(string(item.RouteFallback))),
		InputContractVersion:  item.InputContractVersion,
...
func fromModel(row models.AgentTask) *Task {
	item := &Task{
		...
		PromptHash: row.PromptHash, ModelConfigID: row.ModelConfigID, FinalModelConfigID: row.FinalModelConfigID,
		RouteCandidates: json.RawMessage(stringValue(row.RouteCandidatesJSON)), RouteReason: stringValue(row.RouteReason), RouteFallback: json.RawMessage(stringValue(row.RouteFallbackJSON)),
