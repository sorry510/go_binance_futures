...
###### 类型: 对象
###### 含义: 最近已经结算的 U 本位永续合约资金费率历史，数组按“最新到最旧”排序

- `FundingRate.Data[0]`：最近一次已经结算的资金费率，例如 `0.0001` 表示 `0.01%`。
- `FundingRate.Time[0]`：对应的资金费结算时间，毫秒时间戳。
- 使用固定下标前应先通过 `len(FundingRate.Data)` 判断历史数量是否足够。
- 回测只暴露当前 `asOf` 时刻已经发生的资金费结算，不读取未来费率；实盘读取 Binance 已结算资金费率历史并使用短时缓存。
