...
func TestAgentTaskSyncdbUpgradesExistingSQLiteRows(t *testing.T) {
	...
	orm.RegisterModel(new(AgentTask), new(AgentTaskEvent), new(AgentConversation), new(AgentConversationMessage), new(AgentSkill), new(AgentSkillVersion), new(AgentSkillPermission), new(AgentMCPServer), new(AgentMCPTool), new(AgentMCPResource), new(AgentMCPPrompt), new(AgentMCPPermission), new(AgentMCPSecret), new(AgentMCPOAuthState), new(AgentAlertPipelineTrace), new(AgentMarketEvent), new(AgentMarketEventSource), new(AgentMarketFact), new(AgentMarketSourceStatus), new(MarketConditionHistory), new(MarketDataImportBatch), new(MarketKline1s), new(MarketKline1m), new(MarketKline3m), new(MarketKline5m), new(MarketKline15m), new(MarketKline30m), new(MarketKline1h), new(MarketKline2h), new(MarketKline4h), new(MarketKline6h), new(MarketKline8h), new(MarketKline12h), new(MarketKline1d), new(MarketKline3d), new(MarketKline1w), new(MarketKline1mo), new(MarketFundingRate), new(MarketTrade), new(AgentBacktestDataset), new(AgentBacktestRun), new(AgentBacktestTrade), new(AgentBacktestEvent), new(AgentBacktestEquityPoint), new(AgentMemory), new(AgentWorkflowRun), new(AgentObservation), new(AgentChangeEvent), new(AgentTradeProposal), new(AgentTradeExecution), new(AgentTradeAudit), new(LLMConfig), new(LLMRouterSetting))
